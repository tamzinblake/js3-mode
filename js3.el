...
(defvar js3-RSH 19)
(defvar js3-URSH 20)
(defvar js3-ADD 21)            ; infix plus
(defvar js3-SUB 22)            ; infix minus
(defvar js3-MUL 23)
(defvar js3-DIV 24)
(defvar js3-MOD 25)
(defvar js3-NOT 26)
...
(defvar js3-ASSIGN_LSH 93)     ; <<=
(defvar js3-ASSIGN_RSH 94)     ; >>=
(defvar js3-ASSIGN_URSH 95)    ; >>>=
(defvar js3-ASSIGN_ADD 96)     ; +=
(defvar js3-ASSIGN_SUB 97)     ; -=
(defvar js3-ASSIGN_MUL 98)     ; *=
(defvar js3-ASSIGN_DIV 99)     ; /=
(defvar js3-ASSIGN_MOD 100)    ; %=
  ...
  (let (result head fn parent-chain p elem)
    (dolist (chain chains)
      ...
      (cond
       ...
       ((js3-node-top-level-decl-p head)
        (push chain result))
       ;; check for a this-reference
       ((eq (js3-node-type head) js3-THIS)
        ...
        (if (or (null fn) (js3-nested-function-p fn))
            (push (cdr chain) result)
          ;; else look up parent in function-map.  If not found, discard chain.
          (when (setq parent-chain (and js3-imenu-function-map
...
(defun js3-proper-indentation (parse-status)
  ...
  (save-excursion
    ...
    (cond
     ...
     ((and (not js3-lazy-commas)
      ...
      (let ((spos
		 ...
		 (current-column))
                ...
                ((js3-looking-back (concat "\\<var\\>.*" js3-skip-newlines-re))
     ...
     ;;dot-first
     ((and (not js3-lazy-dots)
	   (= (following-char) ?\.))
      (save-excursion
	(js3-backward-clean)
        (if (not (js3-looking-back (concat "^[ \t]*\\([]})]+\\|.*\\..*\\)"
					   js3-skip-newlines-re)))
            (progn
